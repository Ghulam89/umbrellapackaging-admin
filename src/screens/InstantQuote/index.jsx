...
const InstantQuote = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(
        `${Base_url}/instantQuote/getAll?page=${currentPage}&limit=${limit}&search=${search}`
      )
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/instantQuote/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
      ...
      <section className="mb-20 mt-5 text-gray-800">
        <div className="block rounded-lg shadow-lg bg-white">
          <div className="overflow-x-scroll overflow-y-visible w-full">
            <table className="min-w-full mb-0" style={{ minWidth: '1200px' }}>
