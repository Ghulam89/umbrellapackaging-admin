...
import AddCatalogueProducts from "./AddCatalogueProducts";
const CatalogueProducts = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/catalogueProduct/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
        console.log(res);
  ...
  const removeFunction = (id,productId) => {
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/catalogueProduct/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {

                 const params = new FormData()
              
                        params.append('hasSpecialPrice',false)
                        params.append('specialPrice','')
                      axios.put(`${Base_url}/product/update/${productId}`,params).then((res)=>{
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Catalogue Products</h2>
        <Button
      ...
      <AddCatalogueProducts isModalOpen={isUpdateOpen}
            ...
            <table className="min-w-full mb-0">
              <thead className="bg-primary rounded-lg">
                <tr>
                  ...
                  <th className="text-sm text-white font-bold px-6 py-4">Catalogue Category</th>
                  <th className="text-sm text-white font-bold px-6 py-4">Product</th>
                  ...
                  <tr key={item.id} className="bg-white text-center border-b">
                    ...
                    <td className="text-sm font-normal px-6 py-4">
                      <div className="flex gap-2 justify-center items-center">
                        ...
                        />
                        <img
                          onClick={() => removeFunction(item.id,item?.productId)}
