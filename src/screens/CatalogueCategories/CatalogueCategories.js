...
import AddCatalogueCategories from "./AddCatalogueCategories";
const CatalogueCategories = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/catalogueCategory/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/catalogueCategory/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Catalogue Categories</h2>
        <Button
      ...
      <AddCatalogueCategories isModalOpen={isUpdateOpen}
