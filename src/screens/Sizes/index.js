...
const Sizes = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/size/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/size/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Sizes</h2>
        <Button
      ...
      <AddSizes  isModalOpen={isUpdateOpen}
