...
const Clearance = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(
        `${Base_url}/clearance/get?page=${currentPage}&limit=${limit}&search=${search}`
      )
      .then((res) => {
        console.log(res);
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Clearance</h2>
        <Link to={"/add-clearance"}>
          <Button className="bg-primary py-2.5" label="Add" />
        </Link>
      </div>
                  ...
                  <tr key={item.id} className="bg-white text-center border-b">
                    ...
                    <td className="text-sm font-normal px-6 py-4">
                      <div className="flex gap-2 justify-center items-center">
                        <Link to={`/edit-clearance/${item?.id}`}>
                          <img
