...
const Catalogues = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/catalogue/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/catalogue/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Catalogues</h2>
        <Button
      ...
      <AddCatalogues isModalOpen={isUpdateOpen}
