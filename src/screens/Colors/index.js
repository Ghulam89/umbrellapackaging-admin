...
const Colors = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/color/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/color/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Colors</h2>
        <Button
      ...
      <AddColors  isModalOpen={isUpdateOpen}
