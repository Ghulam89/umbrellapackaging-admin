  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios({
        ...
        url: isEditMode
          ? `${Base_url}/blogCategory/update/${editData.id}`
      ...
      if (response?.status === 200) {
        ...
        toast.success(response.data.message || "Blog Category saved successfully!");
        fetchSizes();
        resetState();
      } else {
        toast.error(response.data.message || "Failed to save Blog Category.");
      }
    } catch (error) {
      ...
      <div>
        <div className="p-3 flex justify-between items-center">
          ...
          <h1 className="capitalize h4 font-semibold">
            {isEditMode ? "Edit Blog Categories" : "Add Blog Categories"}
          </h1>
          <MdClose
