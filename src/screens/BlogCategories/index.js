...
const BlogCategories = () => {
  ...
  const fetchSizes = () => {
    axios
      .get(`${Base_url}/blogCategory/get?page=${currentPage}&limit=${limit}&search=${search}`)
      .then((res) => {
  ...
  const removeFunction = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${Base_url}/blogCategory/delete/${id}`)
          .then((res) => {
            if (res.status === 200) {
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        <h2 className="main_title">Blog Categories</h2>
        <Button
      ...
      <AddBlogCategories isModalOpen={isUpdateOpen}
